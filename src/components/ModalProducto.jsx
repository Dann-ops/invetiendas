...
export default function ModalProducto({ cerrar, producto, idNegocio, recargar }) {
  // Evaluamos si es edición real basándonos estrictamente en la existencia de la llave primaria
  ...
  // Control aislado del estado de apertura de la cámara
  const [camaraAbierta, setCamaraAbierta] = useState(false);

  // CORRECCIÓN CLAVE: Quitamos 'idNegocio' de las dependencias. 
  // Ahora el formulario solo se reiniciará si cambia el objeto físico del 'producto'.
  useEffect(() => {
    ...
    setFormData({
      ...
      stock: producto?.stock || 0,
      stock_minimo: producto?.stock_minimo || 5,
      id_negocio: idNegocio || producto?.id_negocio || ''
    });
  }, [producto]); // <-- Solo vigila si cambia el producto

  const manejarCambio = (e, key) => {
    const valor = e.target.type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value;
    setFormData(prev => ({ ...prev, [key]: valor })); // Uso seguro del estado anterior en React
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#263238]/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300 relative flex flex-col max-h-[90vh]">
        ...
        <div className="p-8 pb-4 flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-gray-400">ID Negocio: {formData.id_negocio || idNegocio}</p>
          </div>
          
          <button
            type="button"
            onClick={() => setCamaraAbierta(true)}
            className="bg-[#263238] text-white px-4 py-2 rounded-xl text-[11px] font-bold hover:bg-[#1a2327] transition-all flex items-center gap-1.5 shadow-sm"
          >
            📷 Escanear
          </button>
        </div>
      ...
      {/* RENDERIZADO DEL ESCÁNER SOBRE EL MODAL */}
      {camaraAbierta && (
        <ScannerCamara 
          cerrar={() => setCamaraAbierta(false)} 
          onScanSuccess={(codigoDetectado) => {
            setFormData(prev => ({ ...prev, codigo_barra: codigoDetectado }));
            setCamaraAbierta(false); // Cierra la cámara automáticamente al detectar código
          }} 
        />
      )}
    </div>
